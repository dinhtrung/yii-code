	...
	protected function renderFilterCellContent()
	{
		if($this->filter!==false && $this->grid->filter!==null && $this->name!==null && strpos($this->name,'.')===false)
		{
			...
			if ($this->filter=='range') {
				echo CHtml::tag('div', array(), "<span style='". $this->dateLabelStyle ."'>". $this->fromText ."</span>" . CHtml::activeTextField($this->grid->filter, $this->name.'[range][from]', array('style'=>$this->dateInputStyle, 'class'=>'filter-date')));
				echo CHtml::tag('div', array(), "<span style='". $this->dateLabelStyle ."'>". $this->toText ."</span>". CHtml::activeTextField($this->grid->filter, $this->name.'[range][to]', array('style'=>$this->dateInputStyle, 'class'=>'filter-date')));
			}
			else {
				echo CHtml::tag('div', array(), CHtml::activeTextField($this->grid->filter, $this->name.'[range][to]', array('class'=>'filter-date')));
			}
