...
class ArrayModel extends CModel
{
	...
	*/
	public function __construct($scenario = 'insert', $filename = NULL)
	{
		if (! $filename) self::$filename = 'application.data.' . get_class($this);
	...
	*/
	public function getData()
	{
		...
		{
			if (! file_exists(Yii::getPathOfAlias($this->fileName()).'.php')){
				$this->setData(array());
			}
	...
	private function dataToAttributes($structure, $data)
	{
		...
		foreach($structure as $key => $attribute)
		{
			...
			else
			{
				...
				if(is_array($data))
				{
					$attributeData[$attribute] = array_key_exists($key, $data)?$data[$key]:NULL;
				}
				else
				{
	...
	*/
	...
	/**
	 * Default rules is to permit all attributes
	 */
