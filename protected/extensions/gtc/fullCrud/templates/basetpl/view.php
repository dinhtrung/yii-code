...
<h1><?php echo "<?php echo Yii::t('app', 'View');?>" ?> <?php echo "<?php echo Yii::t('app', '".$this->modelClass."');?>" . " #<?php echo \$model; ?>"; ?></h1>
	...
	if ($column->isForeignKey) {
		...
		foreach ($this->relations as $key => $relation) {
			if ((($relation[0] == "CHasOneRelation") || ($relation[0] == "CBelongsToRelation")) && $relation[2] == $column->name) {
				...
				echo "\t\t\t'value'=>(\$model->{$key} !== null)?CHtml::link(\$model, array('{$controller}/view','id'=>\$model->{$key}->id)):'n/a',\n";
				echo "\t\t\t'type'=>'html',\n";
			}
		}
		echo "\t\t),\n";
		...
		echo "\t\t\t'type'=>'link',\n";
		echo "),\n";
	} else if ($column->name == 'createtime' or $column->name == 'updatetime' or $column->name == 'timestamp') {
		echo "array(
					'name'=>'{$column->name}',
					'value' =>\$locale->getDateFormatter()->formatDateTime(\$model, 'medium', 'medium')),\n";
...
foreach (CActiveRecord::model(Yii::import($this->model))->relations() as $key => $relation) {
	if ($relation[0] == 'CManyManyRelation' || $relation[0] == 'CHasManyRelation') {
		...
		echo "\n\n<h2>";
		echo "<?php echo CHtml::link(Yii::t('app','{relation}',array('{relation}'=>'" . ucfirst($key) . "')), array('" . ucfirst($relation[1]) . "/admin'));?>";
		echo "</h2>\n";
	...
	if ($relation[0] == 'CHasOneRelation') {
		...
		$suggestedtitle = $this->suggestName($model->tableSchema->columns);
		echo "\n\n<h3>";
		echo "<?php echo CHtml::link(Yii::t('app','{$key}')),'XXX');?>";
		echo "</h3>\n";
