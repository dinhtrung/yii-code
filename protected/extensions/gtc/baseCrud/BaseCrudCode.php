...
class BaseCrudCode extends CrudCode {
	public $authtype;
	...
	public function generateCFormField($model, $column) {
		...
		if (strtoupper($column->dbType) == 'TINYINT(1)'
			...
			return array(
				...
				'options'	=>	array(
					 'dateFormat'=>Yii::t('app', 'jui.datepicker.dateFormat'),
					 ),
				);
		} else if (strtoupper($column->dbType) == 'DATETIME') {
			return array(
				"class" =>	'ext.widgets.datetimepicker.CJuiDateTimePicker',
				'options'=>array(
					 'dateFormat'=>"Yii::t('app', 'jui.datepicker.dateFormat')",
				 ),
			);
