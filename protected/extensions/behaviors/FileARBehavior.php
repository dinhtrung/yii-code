...
class FileARBehavior extends CActiveRecordBehavior {
	...
	/**
	 * default separator when attributeForName is an array and must be implodeed.
	 */
	...
	protected function getFileName() {
		if (!isset($this->_fileName)) {
			...
			if (!is_array($this->attributeForName)) $partName = $this->owner->{$this->attributeForName};
				...
				$partName = implode($this->attributeSeparator, $partName);
			}
	...
	protected function getAnyExistingFilesName($path, $fname) {
		...
		return $this->getExistingFilesName($path, $fname.'{'.implode(',', $suffixes).'}');
	}
