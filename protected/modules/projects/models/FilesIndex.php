 ...
 *
 * The followings are the available columns in table 'files_index':
 * @property integer $file_id
 * @property string $word
 * @property integer $word_placement
 */
class FilesIndex extends BaseActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return FilesIndex the static model class
	 */
	...
	protected function createTable(){
		$columns = array(
			'file_id' => 'integer',	// 
			'word' => 'string',	// 
			'word_placement' => 'integer',	// 
		);
		$this->getDbConnection()->createCommand(
			$this->getDbConnection()->getSchema()->createTable($this->tableName(), $columns)
		)->execute();
		$this->getDbConnection()->createCommand(
			$this->getDbConnection()->getSchema()->addPrimaryKey('file_id_word_word_placement', $this->tableName(), 'file_id, word, word_placement')
		)->execute();
	}
}