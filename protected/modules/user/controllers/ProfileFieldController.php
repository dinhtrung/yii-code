...
class ProfileFieldController extends WebBaseController
{
	...
	var fieldType = {
			...
			'BOOL':{
				...
				'val':{
					'field_size':0,
					'default':0,
					'range':'1==".Yii::t('user', 'Yes').";0==".Yii::t('user', 'No')."',
					'widgetparams':''
				}
			},
			'BLOB':{
	...
	function showWidgetList(type) {
		...
		$('div.widget select').append('<option value=\"\">".Yii::t('user', 'No')."</option>');
		if (wgByType[type]) {
	...
	// show all function
	$('div.form p.note').append('<br/><a href=\"#\" id=\"showAll\">".Yii::t('user', 'Show all')."</a>');
